...
7.  At this point, it’s not hard to add in tooltips that give details of
    the series closest to the mouse. We will use a Voronoi mouseover, so
    that it’s not necessary to hover exactly over each path (which can
    be quite narrow). We define the Voronoi neighborhoods using
    `d3.Delaunay` together with the `x` and `y` scales, just like in our
    [earlier scatterplot
...
8.  In `update_series()`, we use `delaunay.find()` to look up the date
...
9.  Path generators are just one example of a D3 function that maps raw
...
10. Specifically, we can encode the output of the “stacked” array as a
...
11. This area generator can be used to create the D3 streamgraph by
...
12. Just to show how flexible D3 can be, let’s study two more exotic
...
13. This figure can be generated by (i) appending rectangles for each
...
14. Given these scales, we can enter one rectangle per event in the
...
15. Next, let’s consider Bump charts. These are often useful for
...
16. We won’t go over all the steps used to create this visualization
...
17. Perhaps the most challenging part of creating this data
...
18. This is the main logic of the visualization, but this example is
    ...
    Finally, they use a Voronoi mouseover, like what we used in our
    electricity example above.
