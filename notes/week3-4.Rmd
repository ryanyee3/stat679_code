...
Faithful app, which is the default app created when we use Rstudio to create a
new Shiny app.
...
1. The main way to interface to this library in `Shiny` is to use the `bs_theme`
function within the `theme` argument of of `fluidPage` (it actually works for a
variety of UI objects, like `navbarPage` or `fixedPage`). The simplest way to
customize a page’s theme is to set the `bootswatch` argument, which globally
changes the theme to reflect one of a set of predefined `bootswatch` themes —
you can see the choices [here](https://bootswatch.com/).
...
customize a variety of color and font options.
