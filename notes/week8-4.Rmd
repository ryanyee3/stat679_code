...
1. Here is a somewhat more complex version of the above map where each county is
assumed to be associated with a (random) time series. Redrawing the lower and
upper bounds on the time series changes which counties are highlighted. Though
the implementation is more involved, there are no concepts used here that we
haven't covered at some point in the class.
