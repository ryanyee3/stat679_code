...
[Recording](),  [Rmarkdown](https://github.com/krisrs1128/stat679_code/blob/main/notes/week3-3.Rmd)
...
1. Viewed more abstractly, we are going to use a brush to linking the histogram
and the scatterplot views. We will be able to evaluate the change in a
...
and a screenshot of the initial interface is below.
...
`aes_string` to use variable names encoded in strings. The `plot_overlay`
function provides the histogram bar heights for variable `v` after brushing over
the movies in `selected_`.
