...
from Brasilia, polygons of glacier boundaries in Nepal, and locations of
...
```{r}
im <- rast("/Users/ksankaran/Desktop/lake_labeller/analysis/G081089E30084N_2020-05-16.tif") %>%
  stretch()
...
```{r}
tm_shape(subset(im, 2:4)) +
  tm_rrgb() +
  tm_shape(glaciers) +
  tm_polygons(fill = "Thickness") +
  tm_layout(legend.position = c("left", "bottom"))
```
...
bar scales (`tmap_bar`) and compasses (`tmap_compass`). Moreover, the themes and
