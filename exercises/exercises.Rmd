...
studies that you would like to visualize this semester. 
    a. What makes you interested in this dataset? Please provide a link, so we
    can use it to create future exercises.
    b. Using whatever software that you are familar with, prepare a
    visualization to answer one question you have based on the data. This will
    not be evaluated except for effort -- the purpose of the problem is to
...
according to different classes and IDs.
    ...
    c. Using a D3, move the circle with the ID “part_b” to location (100, 100)
    d. Using D3, change all the squares of class “part_c” to red.
    e. Using CSS or D3, change all circles of class “part_d” to red, but only if
    they are contained within the group with label “group_part_d”
    
1. [Debugging JS Code] Imagine that you are helping your colleagues debug their
javascript programs. They have explained their implementation goals and
difficulties they have encountered. Your job is to both help them resolve their
specific bug and to briefly explain the data structure or control flow concepts
that will help them avoid these issues in the future. Specifically, for each
part, explain propose a solution to your colleague’s problem and briefly explain
the concept that will help avoid similar bugs.

    a. Goal: I want to try to get the “height” array out of “mountains” below.
    Attempt: Here is my HTML and JS file. This is the part that I think is
    relevant,
    Problem: The console prints the error message,
    b. Goal: I want to change the color of all the circles to blue.
    Attempt: Here is my HTML and JS file. This is the part that I think is relevant,
    Problem: Only the color of the first circle is changing.
    c. Goal: I want to change the color of all the class “highlighted” elements.
    Attempt: Here is my HTML and JS file. This is the part that I think is relevant,
    Problem: Only the color of the first circle is changing.
    d. Goal: I want to change the color of all the class “highlighted” squares.
    Attempt: Here is my HTML and JS file. This is the part that I think is relevant,
    Problem: Nothing is changing.
    e. Goal: I want to move the circle to location 100, 200 on the SVG canvas.
    Attempt: Here is my HTML and JS file. This is the part that I think is relevant,
    Problem: Nothing is changing.
...
d3’s `.join()` function, write a function that updates the visualization from
[Simple bar chart] every time that `update_data()` is called.
    ...
    function update_data(x) {
      x.push(generator()) // add new element to end
      return x.slice(1) // remove first one
    }
    ```
