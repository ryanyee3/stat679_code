...
1. Problem Set 1 is due on Friday (9/30)
1. Thank you to the students who helped fix the `quantile` input in the in-class
demo. Their approach is in the posted rmarkdown.
...
1. Subtleties between outputs we didn't cover in class -- `eventReaction()` and `observeEvent`
...
These kinds of events actually live "outside" the reactive graph. There is
actually a chapter about this in the [Mastering Shiny
...
1. Using a function within a server outside of a `render` or `reactive` context will fail
...
Q: Subtleties between outputs we didn't cover in class -- `eventReaction()` and `observeEvent`
...
Questions,
  ...
  * Can you imagine alternatives that would convey more information?
  * How would you begin implementing this? Which examples from the notes may be
  relevant?

Take 7 minutes to discuss with a neighbor, then we will regroup and share ideas.
